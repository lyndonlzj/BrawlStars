        ...
        if (!modsessions.includes(name)) {
          ...
          if (creator === userID) {
            ...
            database
              .collection("modules")
              .doc(documentid)
              .update({ sessions: modsessions });
            console.log("updating session array");

            this.usermodulelist.push(modID);
            database
              .collection("users")
              .doc(userID)
              .update({ enrolled: this.usermodulelist });
            this.$router.push({ name: "sessionpage", params: { id: name } });
            return;
          } else {
            alert(
              "You are not the creator of this module. Unable to process change."
            );
          }
        } else {
          //module exist and session exist
          alert(
            "This specific session has already been created within this module"
          );
        }
      } else {
